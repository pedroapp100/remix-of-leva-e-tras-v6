...
export function ComissoesTab({ dateRange }: ComissoesTabProps) {
  ...
  const columns: Column<Comissao>[] = [
    ...
    },
    {
      ...
      header: "Nº Entregas",
      sortable: true,
      cell: (c) => <span className="tabular-nums">{c.entregas}</span>,
    },
    {
      key: "valor_gerado",
      header: "Valor Gerado",
      sortable: true,
      cell: (c) => <span className="tabular-nums">{formatCurrency(c.valor_gerado)}</span>,
    },
    {
      key: "tipo_comissao",
      header: "Tipo",
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>
          Total de entregas:{" "}
          <strong className="text-foreground">{totalEntregas}</strong>
        </span>
        <span>
          Total de comissões:{" "}
      ...
      <p className="text-xs text-muted-foreground">
        ⚠️ Comissões calculadas SOMENTE sobre{" "}
        <code className="bg-muted px-1 rounded">receita_operacao</code>, excluindo créditos de loja.
      </p>
      <DataTable
        data={comissoes}
        columns={columns}
