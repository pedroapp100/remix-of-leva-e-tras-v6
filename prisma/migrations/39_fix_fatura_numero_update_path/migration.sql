-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 39_fix_fatura_numero_update_path
--
-- PROBLEMA:
--   Quando o RPC concluir_fatura_entrega atualiza uma fatura já existente
--   (caminho UPDATE), a variável v_fatura_numero nunca era preenchida.
--   O retorno trazia fatura_numero = '' e o histórico da solicitação
--   registrava "Conciliação administrativa realizada" em vez de
--   "Fatura FAT-XXXX gerada".
--
-- SOLUÇÃO:
--   No RETURNING do UPDATE, buscar também a coluna `numero` da fatura
--   e armazená-la em v_fatura_numero, assim o retorno sempre inclui o número.
-- ─────────────────────────────────────────────────────────────────────────────
