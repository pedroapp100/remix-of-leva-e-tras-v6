-- Atomic RPC: creates/updates fatura + inserts lancamentos + inserts historico
-- in a single transaction. Called by the client after concluding a solicitação
-- for faturado clients.
-- v2: includes auto-close logic for por_entrega frequency.

CREATE OR REPLACE FUNCTION concluir_fatura_entrega(
  p_fatura_id       UUID,       -- existing fatura ID to update, or NULL to create new
  p_sol_id          UUID,
  p_cliente_id      UUID,
  p_cliente_nome    TEXT,
...
BEGIN
  ...
  -- Reads the client's billing config and closes the fatura
  -- atomically if the delivery threshold has been reached.
  SELECT
    c.ativar_faturamento_automatico,
    c.frequencia_faturamento::TEXT,
    c.numero_de_entregas_para_faturamento
  INTO v_auto_enabled, v_freq, v_threshold
  FROM clientes c
