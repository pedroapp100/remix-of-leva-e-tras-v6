-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 54_concluir_fatura_entrega_pos_reabertura
--
-- PROBLEMA:
--   reabrir_entrega_faturada (migration 52/53) reverte corretamente uma entrega
--   faturada errada — mas reconciliar essa mesma entrega de novo, com os valores
--   corrigidos, não atualiza a fatura. O guard no topo de concluir_fatura_entrega
--   (`IF EXISTS (SELECT 1 FROM lancamentos_financeiros WHERE solicitacao_id = ...)`)
--   encontra o lançamento antigo — que nunca pode ser apagado, lancamentos_financeiros
--   é imutável por trigger — e retorna 'already_processed' sem tocar em nada.
--   Mesmo sem esse guard, o INSERT de um novo lançamento colidiria com o índice
--   único uq_lancamentos_sol_tipo (solicitacao_id, tipo) e seria descartado via
--   ON CONFLICT DO NOTHING.
--
-- SOLUÇÃO:
--   1. Nova coluna solicitacoes.reaberta_em — setada por reabrir_entrega_faturada,
--      funciona como flag "esta entrega tem uma reabertura pendente de reconciliar".
--   2. concluir_fatura_entrega passa a checar essa flag antes do guard original:
--      se setada, aplica os valores novos como um ajuste de correção em
--      ajustes_financeiros (mesmo padrão já usado por admin_corrigir_credito_loja,
--      migration 42) e atualiza os totais da fatura diretamente — sem nunca tentar
--      inserir um segundo lançamento para o mesmo (solicitacao_id, tipo). Zero
--      mudança em lancamentos_financeiros, no trigger de imutabilidade, ou no
--      índice único.
--
-- As definições abaixo replicam a definição REAL hoje em produção (lida via
-- pg_get_functiondef — os arquivos das migrations 37/52/53 já estavam parcialmente
-- desincronizados do banco, ver reabrir-entrega-faturada.md), com o mínimo de
-- diff necessário para a correção.
-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE solicitacoes ADD COLUMN reaberta_em TIMESTAMPTZ NULL;

-- ── reabrir_entrega_faturada: + 1 linha (reaberta_em = now()) ──────────────────
...
-- ── concluir_fatura_entrega: + p_usuario_id e bloco de correção pós-reabertura ──
...
BEGIN
  ...
  IF v_reaberta_em IS NOT NULL AND v_lanc_fatura_id IS NOT NULL THEN
    ...
    IF p_total_taxas > 0 THEN
      INSERT INTO ajustes_financeiros (fatura_id, solicitacao_id, tipo, valor, motivo, usuario_id)
      VALUES (
        v_fatura_id, p_sol_id, 'debito'::tipo_ajuste, p_total_taxas,
    ...
    IF p_total_recebido > 0 THEN
      INSERT INTO ajustes_financeiros (fatura_id, solicitacao_id, tipo, valor, motivo, usuario_id)
      VALUES (
        v_fatura_id, p_sol_id, 'credito'::tipo_ajuste, p_total_recebido,
