-- Migration 34: Registra evento "fatura_lancada" no histórico ao criar nova fatura
-- Problema corrigido:
--   Quando uma fatura nova era criada pela RPC, nenhum evento era gravado em
--   historico_faturas. O histórico começava vazio até a primeira "entrega_adicionada".
-- Fix:
--   Dentro do bloco ELSE (nova fatura), logo após o INSERT INTO faturas,
--   insere um evento tipo 'fatura_lancada' com descrição em linguagem natural pt-BR.
...
BEGIN
  ...
  ELSE
    ...
    -- ── 1a) Histórico: fatura lançada (apenas em faturas novas) ──
    INSERT INTO historico_faturas (fatura_id, tipo, descricao)
    VALUES (
      v_fatura_id,
      'fatura_lancada',
      'Fatura ' || v_fatura_numero
        || ' lançada para ' || p_cliente_nome
        || ' — ' || v_rotas_label
